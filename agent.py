...
try:
    ...
    print("  sounddevice non installe : pip install sounddevice")
    AUDIO_OK = False

# Force le backend ALSA sous Linux (pas de PulseAudio/PipeWire)
import platform
if platform.system() == "Linux":
    # Forcer PortAudio a utiliser ALSA directement
    os.environ.setdefault("SDL_AUDIODRIVER", "alsa")
    os.environ.setdefault("AUDIODEV", "hw:0,0")  # sera ecrase par la selection
    # Indiquer a sounddevice d'utiliser ALSA via la variable PortAudio
...
# =============================================================================
def list_alsa_devices_raw():
    """Liste les cartes ALSA disponibles via /proc/asound/cards."""
    cards = []
    try:
        with open("/proc/asound/cards") as f:
            for line in f:
                ...
                if line and line[0].isdigit():
                    ...
                    name = line.split("[")[1].split("]")[0].strip() if "[" in line else line
...
def list_audio_devices():
    """Liste les entrees audio ALSA disponibles."""
    if not AUDIO_OK:
        return []
    devices = []
    try:
        # Forcer le backend ALSA pour la liste
        devs = sd.query_devices()
        for i, d in enumerate(devs):
            if d["max_input_channels"] > 0:
                # Filtrer les devices virtuels PulseAudio/PipeWire si presents
                name_lower = d["name"].lower()
                is_virtual = any(k in name_lower for k in
                                 ["pulse", "pipewire", "default", "dmix", "dsnoop"])
                devices.append({
                    "index":       i,
                    "name":        d["name"],
                    "channels":    d["max_input_channels"],
                    "sample_rate": int(d["default_samplerate"]),
                    "preferred":   any(k in name_lower
                                      for k in ["emu","0202","usb audio","line"]),
                    "virtual":     is_virtual,
                    "alsa_hw":     "hw:" in name_lower or "plughw:" in name_lower,
                })
...
def find_device(name_hint=None):
    """
    Trouve le device ALSA optimal.
    Priorite : hint > EMU/USB non-virtual > ALSA non-virtual > premier dispo
    """
    ...
    # 2. Prefere EMU/USB non-virtuel
    preferred_real = [d for d in devs if d["preferred"] and not d.get("virtual")]
    if preferred_real:
        return preferred_real[0]["index"]

    # 3. N'importe quel device non-virtuel
    real = [d for d in devs if not d.get("virtual")]
    if real:
        return real[0]["index"]

    # 4. Fallback premier disponible
    return devs[0]["index"]

def print_devices():
    print("\n  PERIPHERIQUES AUDIO DISPONIBLES (ALSA)")
    print("  " + "-"*56)
    # Cartes ALSA brutes depuis /proc/asound/cards
    alsa_raw = list_alsa_devices_raw()
    if alsa_raw:
        print("  Cartes ALSA detectees (/proc/asound/cards) :")
        for idx, name in alsa_raw:
    ...
    for d in list_audio_devices():
        if d.get("virtual"):
            tag = "[VIRTUAL]"
            color = ""
        elif d.get("preferred"):
            tag = "[EMU/USB] <-- RECOMMANDE"
            color = ""
        else:
            tag = "[ALSA]"
            color = ""
        print(f"  [{d['index']:2d}] {d['name'][:42]:<42} "
...
def audio_callback(indata, frames, time_info, status):
    ...
    # Normaliser vers float32 [-1.0, +1.0] selon le type d'entree
    raw = indata[:, 0].copy()
    if raw.dtype == np.int32:
        # 24 bits dans int32 : diviser par 2^31
        mono = raw.astype(np.float32) / 2147483648.0
    elif raw.dtype == np.int16:
        # 16 bits dans int16 : diviser par 2^15
...
def main():
    ...
    try:
        ST.running = True
        # Configuration ALSA directe
        # Si Linux : tenter d'abord en acces direct hw:, sinon plughw:
        stream_kwargs = dict(
        ...
        # Sur Linux ALSA : ajuster le dtype selon la resolution
        if platform.system() == "Linux":
            if ST.bits == 24:
                stream_kwargs["dtype"] = "int32"   # ALSA n'a pas de float32 natif
            elif ST.bits == 16:
                stream_kwargs["dtype"] = "int16"
            else:
